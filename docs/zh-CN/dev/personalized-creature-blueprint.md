...
## 11. 路线复盘与优化判断
截至当前仓库状态，本蓝图整体没有偏离最初目标：它仍围绕“高度通用、可复用、可治理、可演化的 creature 模板生态”展开，并且已经把最初最容易漂移的几条边界压住：
- `root-privileged` 保持控制面职责，没有退化成超级 worker。
- `coordinator -> worker-base -> critic` 的最小主链已经落到 package 与 terrarium 样板中。
- skill 已迁到 package 原生发现路径，可选 skill 也按 `skills:` 显式物化，不再依赖 `.trae` 私有目录。
- `output_wiring`、turn barrier / idle barrier、静默执行与结构化产物保留，符合“低 token、可审计、可复用”的原目标。
- 预算、审批、安全等 cross-cutting concern 仍被约束在 plugin / API / Studio 边界，没有塞进 creature prompt。

但推进路径已经出现几个需要校正的偏移：
1. **状态叙述滞后**：文档仍写 `Phase A`，但 `T4-T10`、`T20-T32`、`T36-T37` 已经完成，实际已经越过基线校正与最小主链阶段。
2. **CLI 路线权重偏高**：`T20-T28` 很完整，但统一能力路由还没有真正把 built-in tools、MCP、CLI provider 放进同一执行策略；后续若继续围绕 CLI 微调，会偏离“通用 creature 模板生态”的目标。
3. **验证资产还没充分产品化**：大量 `verify_t*.py` 已能证明单点设计，但只有少量进入 `tests/unit/`，回归保护不足，后续重构容易把已完成能力打碎。
4. **协议文本优化收益递减**：`T9` 已结构稳定，`T10` 净收益波动。继续加厚 prompt / skill 文本会增加 token 成本，不如转向明确触发条件、失败样本归档与回归测试。
5. **记忆与治理层开始成为瓶颈**：主链已经跑通，但 `memory schema`、`curator`、`approval-gate`、`budget-policy`、`audit-guard` 尚未落地，长期个性化与可控演化还没有闭环。
...
由此得到的当前判断：
- **保留**：最小主链、package skill 路线、可选 skill 可移除、静默执行、结构化协议、正式完成判定。
- **收缩**：继续扩写 handoff / review prompt 的优先级；除非有失败样本和回归测试证明收益，否则不再作为主线。
- **前移**：统一能力路由、回归保护、发布治理、memory schema。
- **延后**：curator 具体实现、治理插件封装、evolver 原型。
...
- [x] `T36` 建立长链路 turn barrier / idle barrier 稳定层
  - 状态：已完成
  ...
  - 备注：已在 `LocalTerrariumService` 增加 `completion_scope="graph"` 与图级 idle barrier，`OutputLogCapture` 保留结构化 activity metadata，`run_t8_*` 脚本改为直接从 turn 输出和 activity 证据提取 `result_path`，不再回退到 `mtime` 扫描
- [x] `T37` 建立模块自描述与配置项自动写回机制
  - 状态：已完成
  ...
  - 备注：已为 `cli_invoke`、`provider_select`、`result_feedback`、`lab_report` 加入默认配置声明，提供 `sync_test_kit_module_configs.py`，并补齐“写入缺省值但不覆盖现有手工配置”的回归测试

### 任务组 D. Skill 协议层与 2.0 发现路径
- [x] `T9` 实现 `structured-handoff` skill
  - 状态：已完成
  - 解决问题：跨 creature 交接容易语义丢失
  - 适合模块：`skill`
  - 交付物：handoff 模板与使用规范
  - 完成标准：`coordinator` 和 `worker` 都使用同一交接协议
  - 备注：已新增 `examples/test-kit/skills/structured-handoff/SKILL.md`，在 package manifest 中发布，并通过 `coordinator` 的 `skills:` 显式挂载；同时补充 `verify_t9_t10_protocol_skills.py` 与长链路 demo 脚本验证长上下文压缩场景。近期已进一步补齐长链路 parser 对 ````task_card`、````task_card` 等命名 fence 的兼容性，并通过收紧 skeleton 显著改善 `T9` 的结构完整性；最近连续三轮 demo 中 `skill_guided` handoff 均保持 `structural=True`、`gate=PASS`、`missing fields=none`，可视为阶段性企稳。后续仍可围绕 bucket 分数与语义厚度做小幅优化，但当前不再作为最高优先级。
- [x] `T10` 实现 `review-protocol` skill
  - 状态：已完成
...
- [ ] `T34` 将 `verify_t2x_*.py` 纳入回归保护
  ...
  - 备注：下一轮优先级升为 `P0`；优先覆盖 `T4-T10`、`T21-T25`、`T30-T32`、`T36-T37` 的关键不变量，再决定是否纳入 CI
...
- 当前重点：先完成 `T33` 统一能力路由的可执行化，以及 `T34` 关键验证脚本的一键回归保护
- 当前下一步：`P0 = T33 + T34`；`P1 = T35 + T12`；随后再推进 `T11-T17`
- 当前判断：入口控制面、任务编排层、执行骨架、结构化评审层、最小 terrarium、package skill 发现路径、可选 skill 规则、静默执行、barrier 与模块自描述已经建立。路线没有偏离最初目标，但如果继续优先优化 CLI / protocol 文本，会开始偏离；下一轮必须转向统一路由、回归保护、发布治理和 memory schema
