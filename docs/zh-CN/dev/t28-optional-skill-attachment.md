...
- `.trae/skills/` 这类项目级 skill 适合做“选型入口”和“环境级可用能力”，但不应被误当成唯一的 per-creature 挂载机制。
...
- 项目级 skill 选择目录：
  - `.trae/project-skill-library/creature-creation/`
- 通用策略文件：
  - `attachment-policy.yaml`
- 当前 skill 清单：
  - `catalog.yaml`

推荐以后都按这个流程落地：

1. 真实可调用 skill 放在标准发现路径
2. 项目级目录只负责“选什么、怎么挂、能不能删”
