...
class TestOutputLogCapture:
    ...
    def test_on_activity_with_metadata_logs_metadata(self):
        fake = _FakeOutput()
        c = OutputLogCapture(fake)
        c.on_activity_with_metadata(
            "tool_done",
            "[cli_invoke] OK",
            {"output": "result_path=E:\\x\\result.json", "job_id": "job-1"},
        )
        e = c.get_entries()[0]
        assert e.metadata["activity_type"] == "tool_done"
        assert e.metadata["job_id"] == "job-1"
        assert "result_path=" in e.metadata["output"]
