...
`test-kit` now publishes two protocol-oriented skills through package `skills:`:
...
- `coordinator` mounts `structured-handoff`
- `critic` mounts `review-protocol`
...
- baseline prompt-only behavior
- skill-guided behavior via the actual skill-injection path
