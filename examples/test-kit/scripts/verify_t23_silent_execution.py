...
def run_success_case(working_dir: Path, execute_cli_invocation) -> dict:
    artifact_rel = "tmp/t23-success.txt"
    artifact_path = working_dir / artifact_rel
...
def run_process_error_case(working_dir: Path, execute_cli_invocation) -> dict:
...
def run_missing_artifact_case(working_dir: Path, execute_cli_invocation) -> dict:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify T23 silent execution protocol."
    )
    parser.add_argument(
        "--repo-root",
    ...
    with tempfile.TemporaryDirectory(prefix="t23-silent-execution-") as temp_dir:
        working_dir = Path(temp_dir).resolve()
        success_record = run_success_case(working_dir, execute_cli_invocation)
        error_record = run_process_error_case(working_dir, execute_cli_invocation)
        missing_record = run_missing_artifact_case(working_dir, execute_cli_invocation)
        ...
        check(
            success_record["artifact_paths"], "success case should record artifact path"
        )
        check(
            success_record["stdout_summary"] == "silent success",
            "success summary should be compact",
        )
        check(
            success_record["diagnostic_excerpt"] == "",
            "success case should not upgrade to diagnostics",
        )

        check(error_record["success"] is False, "process error case should fail")
        check(
            error_record["error_kind"] == "process_error",
            "process error should be classified",
        )
        check(error_record["retryable"] is True, "process error should be retryable")
        check(
            "boom" in error_record["diagnostic_excerpt"],
            "process error should expose minimal diagnostic",
        )

        check(missing_record["success"] is False, "missing artifact case should fail")
        check(
            missing_record["error_kind"] == "artifact_missing",
            "missing artifact should be classified",
        )
        check(
            missing_record["retryable"] is False,
            "missing artifact should not be retryable by default",
        )
        check(
            "missing artifacts:" in missing_record["diagnostic_excerpt"],
            "missing artifact should expose artifact hint",
        )

        for record in (success_record, error_record, missing_record):
            for key in ("raw_log_path", "stdout_path", "stderr_path", "result_path"):
                check(
                    Path(record[key]).exists(),
                    f"{key} should exist for {record['provider_name']}",
                )
