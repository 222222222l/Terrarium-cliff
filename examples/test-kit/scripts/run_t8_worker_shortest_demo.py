...
def _latest_result_paths(start_time: float) -> tuple[Path, Path]:
    results = sorted(CLI_RUNS_DIR.glob("*/result.json"), key=lambda path: path.stat().st_mtime, reverse=True)
    for result_path in results:
        if result_path.stat().st_mtime >= start_time - 1:
            invocation_path = result_path.with_name("invocation.json")
            return invocation_path, result_path
    raise AssertionError("No cli_invoke result.json was produced for the shortest-chain run")
...
def _paths_from_execution_packet(execution_packet: str) -> tuple[Path | None, Path | None]:
    for raw_path in re.findall(r"[A-Za-z]:\\[^\r\n\"']+?result\.json", execution_packet):
        result_path = Path(raw_path)
        if result_path.exists():
            invocation_path = result_path.with_name("invocation.json")
            return (
                invocation_path if invocation_path.exists() else None,
                result_path,
            )
    return None, None
...
async def main() -> None:
    ...
    try:
        ...
        invocation_path, result_path = _paths_from_execution_packet(execution_packet)

        if invocation_path is None or result_path is None:
            deadline = time.time() + timeout_s
            while time.time() < deadline:
                try:
                    invocation_path, result_path = _latest_result_paths(start_time)
                    break
                except AssertionError:
                    await asyncio.sleep(1.0)

        if invocation_path is None or result_path is None:
            raise AssertionError("worker shortest-chain run did not produce a cli_invoke result in time")
