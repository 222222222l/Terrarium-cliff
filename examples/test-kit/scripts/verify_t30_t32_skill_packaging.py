...
def main() -> None:
    ...
    manifest_skills = {
        entry["name"]: entry["path"] for entry in manifest.get("skills", [])
    }
    expected_skills = {
    ...
    for skill_name, rel_path in expected_skills.items():
        assert manifest_skills.get(skill_name) == rel_path, manifest_skills
    ...
    assert lab_runner.get("skills") == ["provider-aware-cli-builder"], lab_runner.get(
        "skills"
    )

    terrarium = load_yaml(ROOT / "terrariums" / "lab-smoke" / "terrarium.yaml")
    for channel_name, channel_config in (
        terrarium.get("terrarium", {}).get("channels", {}).items()
    ):
