...
def _latest_result_paths(start_time: float) -> tuple[Path, Path]:
    ...
    raise AssertionError("No cli_invoke result.json was produced for this worker turn")
...
async def _run_worker_turn(
    ...
    *,
    ...
    worker_history = await _chat_turn(service, "worker", task_card)
    ...
    fresh_cli_result_produced = True
    if result_path is None:
        try:
            ...
            if require_new_cli_result:
                raise
    elif result_path.stat().st_mtime < start_time - 1:
        fresh_cli_result_produced = False
